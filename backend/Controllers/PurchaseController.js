...
export default {
    add: async (req, res, next) => {
        try {
            ...
            //Luego de realizar una compra, actualizo el stock
    ...
    list: async (req, res, next) => {
        try {
            ...
            const reg = await Models.Purchase.find({ $or: [{ 'receiptNumber': new RegExp(value, 'i') }, { 'receiptSeries': new RegExp(value, 'i') }] }) //Buscar por numero de recibo o numero de serie ignorando mayusculas y minusculas
                .populate('user', { name: 1 })
                .populate('person', { name: 1 })
    ...
    activate: async (req, res, next) => {
        try {
            const reg = await Models.Purchase.findByIdAndUpdate({ _id: req.body._id }, { status: 1 });

            //Luego de activar una compra, actualizo el stock
    ...
    deactivate: async (req, res, next) => {
        try {
            const reg = await Models.Purchase.findByIdAndUpdate({ _id: req.body._id }, { status: 0 });

            //Luego de desactivar una compra, actualizo el stock
            let details = reg.details;
            details.map((x) => {
